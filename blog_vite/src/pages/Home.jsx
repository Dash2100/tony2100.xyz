...
export default function Home() {
  ...
  return (
    <Page className="w-full items-center p-4 md:p-8 lg:p-16 max-w-325 mx-auto flex flex-col gap-6 md:gap-8">
      {/* Cover */}
      <div className="relative w-full h-50 md:h-67.5 rounded-[25px] md:rounded-[35px] mx-auto bg-[#DBECF8] text-[#4E5969] font-bold shadow-inner overflow-hidden flex items-center justify-center mt-5 md:mt-0">
        <img src="/imgs/home-cover.png" alt="Home Cover"
          className="absolute w-full h-full object-cover z-10" />
        <span className="z-50 flex-col gap-2 md:gap-4 absolute flex w-full">
          <h1 className="text-3xl lg:text-4xl text-center px-4">Tony2100's Life Log</h1>
          <div className="relative w-full min-h-7 md:min-h-9 lg:min-h-12">
            <h1 className="absolute inset-0 text-xl md:text-2xl lg:text-3xl text-center px-4 flex items-start justify-center">
              {typed}
            </h1>
          </div>
        </span>
      ...
      <div className="w-full flex xl:flex-row flex-col gap-6 relative items-start">
        ...
        <div className="flex flex-col gap-6 sticky top-4 self-start w-full xl:w-75 z-20 h-fit shrink-0">
          {/* Featured */}
          <div className="w-full h-fit bg-[#f7fafd] border border-[#4E5969]/20 shadow-inner rounded-[25px] md:rounded-[35px] p-6 flex flex-col gap-4">
