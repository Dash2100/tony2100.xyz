...
export default function Notes() {
  return (
    <Page className="w-full px-5 py-6 sm:p-8 lg:p-16 max-w-225 mx-auto flex flex-col gap-5 md:gap-8">
      <div className="flex flex-col gap-2 sm:gap-3 text-center mt-1 md:mt-0">
        <h1 className="text-2xl md:text-4xl font-bold text-[#4E5969] noto-font">筆記</h1>
        <h2 className="text-sm md:text-2xl text-[#4E5969]/80 noto-font px-2">那些不夠長到寫成一篇文章，卻又想記下來的事</h2>
      </div>
        ...
        {notes.length ? notes.map((n) => (
          <article key={n.slug}
            ...
            <div className="flex flex-col gap-1.5">
              <h2 className="text-[#4E5969] text-[17px] md:text-2xl font-semibold noto-font leading-snug">{n.title}</h2>
