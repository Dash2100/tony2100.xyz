    ...
    <Page className="w-full px-4 py-6 sm:p-8 lg:px-12 lg:py-16 max-w-300 mx-auto flex flex-col gap-5 md:gap-8">
      ...
      <div className="flex flex-col gap-4 sm:gap-5">
        {notes.length ? notes.map((n) => (
          <article key={n.slug}
            className="w-full bg-[var(--card)] border border-[var(--ink)]/20 shadow-inner rounded-3xl md:rounded-[35px] p-5 sm:p-6 md:p-8 flex flex-col gap-3">
            <MdContent html={n.html} className="select-text note-content" />
            <div className="flex justify-end">
              <span className="flex items-center gap-1.5 bg-[var(--card-2)] shadow-inner rounded-lg md:rounded-[10px] px-2.5 py-1 text-[13px] md:text-sm text-[var(--ink)]/90 noto-font">
                <img src="/imgs/icon/date.svg" alt="" className="h-4 w-4" />
                {n.dateText}
              </span>
            </div>
          </article>
        )) : <p className="text-[var(--ink)] noto-font text-center py-10">目前還沒有筆記。</p>}
      </div>
    </Page>
  );
}
